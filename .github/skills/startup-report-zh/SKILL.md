...
2. For each chapter `2–9` of `101-report-document.zh.yaml`, mirror the chapter structure of `101-report-document.yaml`.
   - **Section number remap**: `101` chapter `N` uses sections numbered `N.X`; the corresponding source artifact is `XX = N-1` and its sections are numbered `(N-1).X`. When looking up the matching `XX.zh.yaml` section, translate `N.X` back to `(N-1).X` first. The full mapping is defined in `AGENTS.md` ("Section numbering convention"); failing to remap leaves chapters 2 and 9 untranslated because the lookup keys do not exist in the source.
