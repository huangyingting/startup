...
function checkLedger(run, ledger) {
  ...
  const file = `${run}/90-evidence.yaml`;
...
function checkHighProfileEvidence(run, ledger, card) {
  ...
  if (sourceCount < MIN_HIGH_PROFILE_SOURCES) {
    fail(`${run}/90-evidence.yaml: high-profile company has only ${sourceCount} retained sources; expected at least ${MIN_HIGH_PROFILE_SOURCES} or a documented reason`);
  }
  if (claimCount < MIN_HIGH_PROFILE_CLAIMS) {
    fail(`${run}/90-evidence.yaml: high-profile company has only ${claimCount} claims; expected at least ${MIN_HIGH_PROFILE_CLAIMS} or a documented reason`);
  }
}
...
function checkReportCoverage(run, docs, report) {
  ...
  if (upstreamTables.size && reportTables.size / upstreamTables.size < REPORT_COVERAGE_FLOOR) {
    fail(`${run}/91-full-report.yaml: preserves only ${reportTables.size}/${upstreamTables.size} upstream tables`);
  }
  if (upstreamFigures.size && reportFigures.size / upstreamFigures.size < REPORT_COVERAGE_FLOOR) {
    fail(`${run}/91-full-report.yaml: preserves only ${reportFigures.size}/${upstreamFigures.size} upstream figures`);
  }
  if (missingTables.length) warn(`${run}/91-full-report.yaml: upstream table(s) missing without coverageNotes: ${missingTables.join(', ')}`);
  if (missingFigures.length) warn(`${run}/91-full-report.yaml: upstream figure(s) missing without coverageNotes: ${missingFigures.join(', ')}`);
}
...
function loadCoreArtifacts(run, dir) {
  ...
  for (const [key, file] of [['ledger', '90-evidence.yaml'], ['report', '91-full-report.yaml'], ['card', '92-summary-card.yaml']]) {
...
function checkRun(run) {
  ...
  if (!existsSync(join(dir, '92-summary-card.yaml'))) {
    if (yamlFiles(dir).length) fail(`${run}: partial report folder has YAML files but is missing 92-summary-card.yaml`);
    return false;
  }
  ...
  if (report) checkRenderableData(run, '91-full-report.yaml', report);
