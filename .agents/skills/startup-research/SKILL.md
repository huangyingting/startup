...
4. Make sure `STARTUP_FETCH_LOG_PATH` is exported before any later `fetch-url` invocation in this run.
   - If your environment already exports it (CI exports a workflow-wide trail at `.research-cache/_fetch-log.jsonl`), keep that value — `check-chapter` only needs every cited URL to appear in the trail at least once.
...
5. On failure, fix dimensions in `retryOrder[]` first. Treat root-cause dimensions and global hints before object-by-object cosmetic edits.
   - Honor the `retryPolicy` in [`rules.md`](references/rules.md) → *Agent policy (binding)*: at most **3** retries (`maxChapterRetries: 3`), and each retry must reduce the failure count (`requireMonotonicFailureDecrease: true`). If the budget is exhausted with failures still present, stop and report the chapter as blocked rather than looping.
...
Parallel chapter drafting is allowed, but parallel chapters must converge before finalize. Two facts to remember:
...
When `finalize-report` exits 0, summarize the run for the user using every field listed in [`rules.md`](references/rules.md) → *Agent policy (binding)* → `finalResponseFields`. Pull the values straight from the produced `summary-card.yaml` / `full-report.yaml` / `evidence.yaml`; do not paraphrase or omit fields.
