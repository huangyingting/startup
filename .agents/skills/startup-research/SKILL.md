...
1. **Load the chapter packet.**
   ...
   `--include-context` is recommended from chapter 2 onward: with `--report-folder` set, the packet adds `contextChapters[]` (sections / tables / figures / claimRefs of every chapter listed in `optionalContext`), `cumulativeContext` (running unresolved-question count and restricted-access share from earlier chapters — advisory, does not gate this chapter), and `runCache` (the `disclosure-hint.yaml` and `refresh-context.yaml` written by `new-report.mjs`, when present). Drop the flag only when authoring chapter 1. Pass `--no-workflow` only if you specifically need the raw chapter spec.
2. **Use `packet.chapter` as the chapter brief** (`key`, `order`, `file`, `artifact`, `title`, `mission`, `optionalContext`, `contentRequirements`, `plannedTables`, `plannedFigures`, `evidenceStrategy`, `qualityBar`, `gate`). The packet root also carries `vocabularies` (canonical enums for `sourceType`, `sourceStance`, `claimType`, `questionType`, `enumerationCoverage`, `restrictedAccessStatuses`, etc. — same source `check-chapter` validates against) and `checkDimensions` (validator dimensions in retry-precedence order). Read these instead of memorising literals. When `packet.runCache.disclosureHint` is non-null, adopt its `canonicalEvidenceGaps[]` in chapter 04 (financials); when `packet.runCache.refreshContext` is non-null, treat the prior-run snapshot as background only and re-fetch every volatile fact.
...
5. **Build the evidence ledger.** Convert reviewed URLs into `localEvidence.sources[]` (see `source` in the schema for required fields). Then write `localEvidence.claims[]` (atomic facts) using the schema's `claim` shape; set `answersQuestionRefs: [Q<L>###]` to close the loop on questions.
   ...
   - **High-confidence claims demand corroboration.** When you set `claim.confidence: high`, supply at least `gate.minHighConfidenceCorroboration` `sourceRefs` and at least one of them must be a primary tier source (`sourceType` in `packet.vocabularies.primaryTierSourceTypes` or `reputationTier: high`).
6. **Write the chapter YAML** at `reportFolder/<chapter.file>` per the report schema. Populate `sections[]`, `tables[]`, `figures[]`, `callouts[]`, and `evidenceGaps[]` together — they share the same evidence ledger built in step 5.
   ...
   - **Mark enumeration tables.** When a `plannedTable` has `enumeration: true`, the matching `tables[]` entry must include `enumerationScope: { coverage, basis: "..." }` where `coverage` is one of `packet.vocabularies.enumerationCoverage`. Non-`exhaustive` coverage requires either an `evidenceGap` entry whose `topic` mentions the table or a gap entry whose `relatedTableRefs[]` includes the table id. The table must also be backed by claims pointing to at least `gate.minSourcesPerEnumerationRow` distinct registrable domains.
...
- Local IDs follow `<Type><ChapterLetter><Seq3>` (e.g. `SO001`, `CO045`, `TO008`, `FO002`, `QO003` for the company-overview chapter whose `letter:` is `O`). Type is `S` (source), `C` (claim), `T` (table), `F` (figure), `Q` (researchQuestion). Each chapter generates IDs only with its own chapter letter from `chapters.yaml` — never copy an id from another chapter into this chapter's `claimRefs[]` or prose. If you need a fact established in a different chapter, restate it as a new local claim here (any unused id in this chapter's letter space), give it its own `sourceRefs[]`, and reference that new id; ledger consolidation dedupes equivalent claims at the end. Restating, not copying, is the only correct path. (`check-chapter` flags violations as `crossChapterRefLeak`.)
...
- Source `accessStatus` matters. Sources whose `accessStatus` is in `packet.vocabularies.restrictedAccessStatuses` cannot be the sole `sourceRef` of a high-confidence claim, and the report-level gate fails when more than 30 % of sources fall in those buckets.
