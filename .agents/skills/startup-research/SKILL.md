...
1. Load its per-chapter delta:
   ...
   - **`cumulativeContext` partial-rollup signal** (only present with `--include-context`): the loader projects `cumulativeContext.partial: true` and a `cumulativeContext.warnings[]` entry with `code: 'cumulativeContextPartial'` whenever any earlier chapter file is missing or unreadable (the most common cause is an agent that violated the parallel-mode rule above and passed `--include-context` while siblings are still in flight). When you see `partial: true`, treat `cumulativeUnresolvedQuestions` and `cumulativeRestrictedAccessPct` as incomplete and do not gate authoring decisions on them; the same fields land cleanly during step 7's convergence rerun once every sibling is on disk.
...
`--refresh` rewires the revision graph in addition to producing a normal report. The CLI flow (`create-report-run --refresh…` → multi-match disambiguation → `finalize-report --refresh`) is in *Runtime bootstrap* step 3 and *Finalization* step 3; this section covers the runtime-visible behavior.

1. **Company match is OR-semantics** (used by both `create-report-run` and `link-refresh`): a prior report counts as the same company when its `summary-card.yaml` matches the new invocation on either normalized name **or** normalized registrable domain. Either alone is sufficient.
2. **Detecting refresh mode at runtime:** the loader emits `runtimeContext.runCache` whenever `--report-folder` is supplied — `{ refreshContext: null }` for fresh runs, `{ refreshContext: {…} }` when a prior summary was cached. Test `runtimeContext.runCache?.refreshContext != null`; do not infer refresh from folder name or argv. Read the cached prior summary for context, but treat every value listed in [`rules.md`](references/rules.md) → `volatileFacts` as stale and re-fetch it.
3. **`finalize-report.mjs --refresh` runs `link-refresh.mjs` in two phases:**
   - **`prepare-refresh` (`--prepare-current`)** — after `check-report-meta`, before `build-evidence-ledger`. Writes the *new* run's `revision.status: current`, `revision.refreshOfRunId`, `revision.supersededByRunId: null`, and `revision.refreshReason` onto its `report-meta.yaml`. The prior run is untouched.
   - **`link-refresh` (default mode)** — after `check-report` passes. Flips the prior run's `revision.status` to `superseded`, sets its `supersededByRunId` back-pointer, and reassembles its `summary-card.yaml` / `full-report.yaml`.
4. **Revision authoring rule:** omit the `revision` block from `report-meta.yaml` entirely. Both `link-refresh` phases own every revision field on both runs. Disambiguate prior matches at create time with `create-report-run --refresh-of <priorRunId>`, never by hand-authoring `revision.refreshOfRunId`.
